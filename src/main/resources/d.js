!
    function ()
    {
        ...
        }, t.dataFromLocal = function (e)
        {
            if (local.getItem(e))
            {
                return JSON.parse(local.getItem(e));
            }
...
var Comment = function (e)
    {
        ...
        }, i.registerComEvent = function ()
        {
            ...
            }), $(".comment-sumit").click(function ()
            {
                ...
                if ("c" == t)
                {
                    i.addComment(e).then(i.addCommetLocal);
                }
                else if ("r" == t)
                {
                    var n = $(this).data("id");
    ...
    Detail = function ()
    {
                    ...
                    case 2:
                        if (pinglun_get_num == 1)
                        {
                            ...
                            $.ajax({
                                ...
                                type: "GET",
                                //提交的网址
                                url: pinglun_url + "/pl/?id=" + pinglun_id,
                                success: function (data)
                                {
                                    data = JSON.parse(data);
                                    var back_code = data.code;
                                    var back_msg = data.message;

                                    if (back_code == "403")
                                    {
                                        $(".log_pl .txt").html(back_msg);
                                    }
                                    else if (back_code == "201")
                                    {
                                        $(".no-comment").css("display", "block");
                                        $(".log_pl").hide();
                                    }
                                    else if (back_code == "202")
                                    {
                                        $(".log_pl .txt").html("主人还等什么，快畅聊一番吧～");
                                        $(".log_pl").show();
                                    }
                                    else if (back_code == "203")
                                    {
                                        $(".log_pl .txt").html(back_msg);
                                    }
                                    else if (back_code == "200")
                                    {
                                        $(".log_pl").hide();
                                        var back_list = data.data.list;
                                        var pl_data_len = back_list.length;
                                        var pl_List = "";
                                        for (var pl_i = 0; pl_i <= (pl_data_len - 1); pl_i++)
                                        {
                                            if (back_list[pl_i]['chap'])
                                            {
                                                var back_chap = "<p>来自章节  --  " + back_list[pl_i]['chap'] + "</p>";
                                            }
                                            else
                                            {
                                                var back_chap = "";
                                            }
                                            pl_List += '<div class="comment-item"><div class="info"><span class="avatar"><img src="' + back_list[pl_i]['touxiang'] + '"></span><p class="name vip">' + back_list[pl_i]['name'] + '<i class="i-com-vip"></i></p><p class="comment-time">' + back_list[pl_i]['addtime'] + '</p></div><div class="comment-content"><p class="main-comment">' + back_list[pl_i]['cont'] + '</p></div><div class="reply-box">' + back_chap + '</div></div>';
                                        }
                                ...
                                error: function ()
                                {
                                    pinglun_get_num = 1;
                                    $(".no-comment").css("display", "block");
                                    $(".no-comment .txt").html("评论出错啦，请稍后再试");
                                    $(".log_pl").hide();
        ...
        }, n.readChapterAt = function ()
        {
            ...
            {
                t[o].comic_id == e && (i = t[o].chapter_id);
            }
...
$(".long_pl").click(function ()
{
    ...
    $.ajax({
        ...
        type: "GET",
        //提交的网址
        url: pinglun_url + "/pl/?id=" + pl_id_num + "&page=" + pl_page_num,
        success: function (data)
        {
            data = JSON.parse(data);
            var back_code = data.code;
